...
class SpeechPipeline:
    ...
    async def process(self, audio_path: str, user_id: str = "local_demo",
        ...
        def _refine():
            return self.corrector.refine(raw_text)

        def _fillers_stats_raw():
            # Run on raw text first, will re-run on refined if needed
            return self.filler_service.detect_with_stats(raw_text)

        def _level_raw():
            return self.english_level_model.classify(raw_text, audio_features)

        # All three run simultaneously on CPU thread pool
        refined_text, filler_stats, english_lvl = await asyncio.gather(
            loop.run_in_executor(self._cpu_thread_pool, _refine),
            loop.run_in_executor(self._cpu_thread_pool, _fillers_stats_raw),
            loop.run_in_executor(self._cpu_thread_pool, _level_raw),
        )

...
async def signup(request: Request):
    ...
    try:
        ...
        print(f"✅ [SIGNUP] SUCCESS: {req.email} (name={display_name})")
        
        return {"success": True, "token": token, "user": {"email": req.email, "name": display_name}}

    except Exception as e:
        print(f"💥 [SIGNUP] CRASH: {e}")
...
async def login(request: Request):
    ...
    try:
        body = await request.json()
        print(f"[LOGIN] Body received: {body}")
        req = AuthRequest(**body)
    except Exception as e:
    ...
    try:
        ...
        if not verify_password(req.password, existing["password_hash"]):
            print(f"[LOGIN] ❌ Wrong password for: {req.email}")
...
async def reset_password(request: Request):
    ...
    try:
        ...
        print(f"💥 [RESET-PASSWORD] CRASH: {e}")
